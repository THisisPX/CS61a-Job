...
def reverse_recursive(lst):
    ...
    if len(lst) <= 0 :
        ...
        lst = [lst[len(lst)-1]] + reverse_recursive(lst[0:len(lst)-1])
    return lst
...
def distance(city_a, city_b):
    ...
    lon = get_lon(city_a) - get_lat(city_b)
    return sqrt(lon**2+lat**2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    dis_a = get_lat(city_a)**2 +get_lon(city_b)**2
    dis_b = get_lat(city_a)**2 + get_lon(city_b)**2
    if dis_a >= dis_b :
